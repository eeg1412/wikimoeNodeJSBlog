...
if (shouldBuild) {
  execSync('cd admin && yarn install && yarn build && cd ..', { stdio: 'inherit' });
  execSync(`cd server && yarn install && cd ..`, { stdio: 'inherit' });
  execSync('cd blog && yarn install && yarn build && cd ..', { stdio: 'inherit' });
}

const { result } = concurrently([
  { command: `cd server && yarn run start`, name: 'server', prefixColor: 'blue' },
  { command: `cd blog/build && yarn run ${startCommand}`, name: 'blog', prefixColor: 'green' },
], {
  prefix: 'name',
