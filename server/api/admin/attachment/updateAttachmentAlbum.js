...
module.exports = async function (req, res, next) {
  ...
  // updateMany
  attachmentUtils.updateMany({ _id: { $in: ids } }, params).then(async (data) => {
    ...
    for (let key in albumIdMap) {
      albumUpdatePromise.push(albumUtils.updateOne({ _id: key }, {
        $inc: {
    ...
    // 更新album的count
    await Promise.all(albumUpdatePromise).then(() => {
      // 更新对象album的count
      const toAlbumParams = {
        $inc: {
          count: ids.length
      ...
      // 更新album的count
      albumUtils.updateOne({ _id: albumId }, toAlbumParams).then((data) => {
        res.send({
          data: data
        })
