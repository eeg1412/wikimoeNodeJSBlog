    ...
    backupUtils.save({
      ...
      restoreWorker.on('message', async (message) => {
        ...
        if (message.status === 'success') {
          // 成功
          backupUtils.updateOne({ _id }, {
            status: 1
          }).then(() => {
            adminApiLog.info(`restore backup success`)
          }).catch((err) => {
            adminApiLog.error(`restore backup success, ${logErrorToText(err)}`)
          })
        } else {
          // 失败
          backupUtils.updateOne({
            ...
            adminApiLog.error(`restore backup fail, ${logErrorToText(message.error)}`)
          }).catch((err) => {
