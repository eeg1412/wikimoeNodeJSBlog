...
async function updateCollectionDefaults (model) {
  ...
  const totalCount = await model.collection.countDocuments({ $or: orConditions });

  if (totalCount === 0) {
    console.log(chalk.yellow(`在 ${model.collection.name} 中没有需要更新的文档`));
    return;
  }

  // 查找需要更新的文档
  console.log(chalk.green(`在 ${model.collection.name} 中找到 ${totalCount} 个需要更新的文档`));
