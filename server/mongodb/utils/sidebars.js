...
exports.findOne = async function (parmas) {
  // document查询
  return await sidebarsModel.findOne(parmas);
}

// 查找所有
exports.find = async function (parmas, sort) {
  // document查询
  return await sidebarsModel.find(parmas).sort(sort);
}

// 分页查询
exports.findPage = async function (parmas, sort, page, limit) {
  // document查询
  const list = await sidebarsModel.find(parmas).sort(sort).skip((page - 1) * limit).limit(limit);
