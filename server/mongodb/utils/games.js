...
exports.findPage = async function (parmas, sort, page, limit, projection, options = {}) {
  // document查询
  const q = gamesModel.find(parmas, projection).populate('gamePlatform', '_id name color').populate('screenshotAlbum', '_id name').sort(sort).skip((page - 1) * limit).limit(limit);
  if (options.lean) {
    q.lean();
  }
  const list = await q;
