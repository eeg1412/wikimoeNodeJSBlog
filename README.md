...
| `NUXT_API_DOMAIN` | `http://server:3006`                     | **必填项非常重要！** blog 容器访问 server 容器的地址。默认 compose 部署请保持 `server` 这个服务名。 |
...
| 变量名                         | 例子                                   | 说明                                                                                                                            |
| :----------------------------- | :------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------ |
| `MULTILINGUAL_DOMAIN`          | `http://multilingual.example.com:3016` | server 容器访问多语言站后端的地址。用于源站配置变更后通知多语言站刷新缓存，以及发送多语言评论回复邮件时拉取译文文章和站点配置。 |
| `NUXT_API_MULTILINGUAL_DOMAIN` | `http://multilingual.example.com:3016` | blog 容器代理 `/api/multilingual-blog` 和 `/multilingual-assets` 时使用的上游地址。启用 `/<language-code>` 页面前必须配置。     |

> 这两个地址都必须是 **容器内可访问** 的真实地址。对于当前 compose 文件，`localhost` 只代表容器自身，不能直接指向另一台机器上的多语言站。
...
如果你已经接入多语言站，请先确保多语言站服务已经可用，并且 `.env` 中的 `MULTILINGUAL_DOMAIN` 与 `NUXT_API_MULTILINGUAL_DOMAIN` 已经改成正确地址。
